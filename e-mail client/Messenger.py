...
class mywindow(QtWidgets.QMainWindow):
    def __init__(self):
        global user, password,sender, usr, pasw, list_date
        ...
        self.ui.pushButton_3.clicked.connect(self.re_log)

        self.list_date=["random text"]
        usr, ok = QInputDialog().getText(self, "Авторизация", "Введите логин:", QLineEdit.Normal)
        if ok and usr:
            user=usr
        pasw, ok = QInputDialog().getText(self, "Авторизация", "Пароль:", QLineEdit.Password)
        if ok and pasw:
            password=pasw
            sender=usr
    ...
    def recive(self):
        try:
            threading.Timer(3.0, self.recive).start()  # Перезапуск через 5 секунд
            ...
            if self.list_date[0]==date:
                self.list_date.pop(0)
                print("хихих")
                svertoch.pause()
            else:
                self.list_date.pop(0)
